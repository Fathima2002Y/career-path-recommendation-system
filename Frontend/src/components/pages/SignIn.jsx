...
const SignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error) {
        setLoading(false);
        Swal.fire({ icon: "error", title: "Oops...", text: error || "Failed to sign in" });
        return;
      }

      if (!data || !data.success) {
        setLoading(false);
        Swal.fire({ icon: "error", title: "Oops...", text: data?.message || "Invalid Credentials!" });
        return;
      }
      ...
      setLoading(false);
      Swal.fire({ icon: "error", title: "Oops...", text: error.message || "Failed to sign in" });
    }
  };
      ...
      <motion.div variants={slideIn("left", "tween", 0.2, 1)} className="flex-[0.75] p-8 rounded-2xl">
        ...
        <form className="mt-12 flex flex-col gap-8">
          <label className="flex flex-col">
            ...
            <input
              type="email" id="email" placeholder="What's your email?"
          ...
          <div className="flex gap-10 items-center">
            ...
            <Link to={"/register"} className="py-3 px-8 rounded-xl bg-red-600 w-fit text-black font-bold shadow-md shadow-primary hover:bg-red-400">
              Sign Up
            </Link>
          </div>
        </form>
